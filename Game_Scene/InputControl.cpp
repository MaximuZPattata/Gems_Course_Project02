...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
    ...
    if (mods == 0)
    {
        ...
        {
            CAMERA_POSITION = glm::vec3(322.0f + POSITION_OFFSET, 376.0f + POSITION_OFFSET, 390.f + POSITION_OFFSET);
            gameEngine.MoveCameraTarget(-0.53f, -0.53f, -0.65f);
            gameEngine.yaw = -130.f;
            gameEngine.pitch = -32.f;
            resetCamAttributes();
        }
