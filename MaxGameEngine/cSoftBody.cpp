...
void cSoftBody::FindCenterNodes()
{
	...
	for (cSoftBody::sNode* vertex : this->nodesList)
...
void cSoftBody::CalculateDampingValue(float deltaTime)
{
	...
	//printf("\nDistance : %f || CurrPos : (%f, %f, %f) || OldPos : (%f, %f, %f) ", distance, this->mImaginaryCenterNode->currentPosition.x, 
	//	this->mImaginaryCenterNode->currentPosition.y, this->mImaginaryCenterNode->currentPosition.z, this->mImaginaryCenterNode->oldPosition.x,
	//	this->mImaginaryCenterNode->oldPosition.y, this->mImaginaryCenterNode->oldPosition.z);
...
void cSoftBody::CreateWheelBracing()
{
	...
	//-------------Connect selected vertex with the center vertex---------------
		...
		for (sNode* centerVertex : centerNodesList)
		{
			...
			this->constraintsList.push_back(centerBracingConstraint);
		}
	}
}
...
void cSoftBody::ApplyConstantPosition(glm::vec3 constantPos)
{
	if (constantPos.x != 0.f)
		this->constantPosition.x = constantPos.x;
	if (constantPos.y != 0.f)
		this->constantPosition.y = constantPos.y;
	if (constantPos.z != 0.f)
		this->constantPosition.z = constantPos.z;
}
